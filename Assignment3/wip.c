...
void parsefile(){
  ...
  size_t chunksize=5; // not crazy chunk size chosen
  size_t rest=0;
  int chunks=linecount/chunksize;
...
#pragma omp critical
	{
	  dist=(unsigned short)roundf(sqrtf((cells1[i][0]-cells1[j][0])*(cells1[i][0]-cells[j][0])+
	...
	for(size_t j=0;j<3;j++){
      ...
      // unsigned short dist[linecount][1];
...
#pragma omp critical
  ...
  // should write the distances which occurred at least once and their frequencies
